...
class Cinema
	...
	def group_by_genre
		@movies.inject({}) do |hash, movie|
			hash[movie.genre] ||= [] #necesita inicializarse a array 
			hash[movie.genre] << movie.title  #xq sino es nill, y nill.push (q es = q <<) no existe
			hash                              #pero array.push (osea <<) si existe y lo reconoce
		end
	end
